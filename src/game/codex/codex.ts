...
function hideActionFromCodex(action: ActionRow): boolean {
  if (
    action["Action ID"] === HIDE_FROM_CODEX_ACTION_ID ||
    notesHideFromCodex(action.Notes)
  ) {
    return true;
  }
...
function combatEnemyId(action: ActionRow): string | null {
  ...
  if (action["Target Type"] === "Enemy" && action["Target ID"])
 ...
 */
function includeQuestRewardAsObtainSource(
...
export class CodexIndex {
    ...
    return this.itemOrder
      ...
      .filter(
        (entry) =>
          ...
          this.sorter.itemMatchesName(entry.itemId, needle),
      );
  }
  ...
  enemiesMatching(query = ""): CodexEnemyEntry[] {
    ...
    return this.enemyOrder
      ...
      .filter(
        (entry) => !needle || entry.displayName.toLowerCase().includes(needle),
      );
  }
  ...
  private build() {
    const names = new Map(
      this.db.Items.map((item) => [item["Item ID"], item["Display Name"]]),
    );
    const enemyNames = new Map(
      this.db.Enemies.map((enemy) => [
        enemy["Enemy ID"],
        enemy["Display Name"],
      ]),
    );
    const skills = new Map(
      this.db.Skills.map((skill) => [skill["Skill ID"], skill["Display Name"]]),
    );
    ...
    for (const action of this.db.Actions) {
      ...
      const obtainFromAction = (
        extra: Partial<CodexObtainSource> = {},
      ): CodexObtainSource =>
        enemyId
          ? {
              kind: "enemy",
    ...
    for (const recipe of this.db.Recipes) {
      ...
      addCraft({
        ...
        facilityName:
          facilities.get(recipe["Facility ID"])?.["Display Name"] ?? null,
        output: {
          itemId: outputId,
    ...
    for (const project of this.db.Projects) {
      ...
      addCraft({
        ...
        facilityName:
          facilities.get(project["Facility ID"])?.["Display Name"] ?? null,
        output: {
          itemId: outputId,
          displayName: names.get(outputId) ?? outputId,
    ...
    const itemCategory = new Map(
      this.db.Items.map((item) => [item["Item ID"], item.Category]),
    );
    for (const quest of this.db.Quests) {
      const rewardId =
        typeof quest["Reward Item ID"] === "string"
          ? quest["Reward Item ID"]
          : null;
      if (!rewardId) continue;
      if (
        !includeQuestRewardAsObtainSource(itemCategory.get(rewardId), rewardId)
      )
        continue;
      const qty =
        typeof quest["Reward Item Quantity"] === "number"
          ? quest["Reward Item Quantity"]
          : null;
      const title =
        typeof quest["Display Name"] === "string"
          ? quest["Display Name"]
          : String(quest["Quest ID"] ?? "Quest");
      const questId =
        typeof quest["Quest ID"] === "string" ? quest["Quest ID"] : null;
      addObtain(rewardId, {
        kind: "quest",
        title,
        questId,
    ...
    for (const starter of this.db.RaceStartingItems) {
      const race = this.db.Races.find(
        (row) => row["Race ID"] === starter["Race ID"],
      );
    ...
    itemIds.sort((a, b) =>
      this.sorter.compareGrouped({ itemId: a }, { itemId: b }),
    );
    for (const itemId of itemIds) {
      ...
      this.itemsById.set(itemId, {
        ...
        statLines: [
          ...equipmentTooltipStatLines(
            equipmentForItemId(this.db, itemId),
            this.db,
          ),
          ...(isSpellItem(this.db, itemId)
            ? spellTooltipLines(this.db, item, itemId)
            : []),
        ],
    ...
    for (const enemy of this.db.Enemies) {
      ...
      if (
        /squidling/i.test(enemy["Display Name"]) ||
        /squidling/i.test(notes)
      ) {
        fishingEnemyIds.add(enemy["Enemy ID"]);
      }
    }
    ...
    enemies.sort((a, b) => {
      ...
      return a["Display Name"]
        .toLowerCase()
        .localeCompare(b["Display Name"].toLowerCase());
    });
    for (const enemy of enemies) {
      const tableId = enemy["Reward Table ID"];
      const drops = (tableId ? (tableItems.get(tableId) ?? []) : []).filter(
        (drop) =>
          drop.itemId !== GOLDEN_SPUD_ITEM_ID &&
          this.itemsById.has(drop.itemId),
      );
      ...
      this.enemiesById.set(enemy["Enemy ID"], {
        ...
        xpSkillLabel: fishingEnemyIds.has(enemy["Enemy ID"])
          ? "Fishing"
          : "Combat",
    ...
    actions.sort((a, b) => {
      ...
      const level =
        (a["Proficiency Level"] ?? 0) - (b["Proficiency Level"] ?? 0);
      if (level !== 0) return level;
      return a["Display Name"]
        .toLowerCase()
        .localeCompare(b["Display Name"].toLowerCase());
    ...
    for (const action of actions) {
      ...
      const targetId =
        action["Target Type"] === "Item" && action["Target ID"]
          ? action["Target ID"]
          : null;
  ...
  private actionLocations(
    locations: Map<string, string>,
  ): Map<string, CodexLocationRef[]> {
    ...
    for (const activity of this.db.Activities) {
      ...
      for (const actionId of poolActions.get(poolId) ?? []) {
        ...
        if (!list.some((row) => row.locationId === loc.locationId))
          list.push(loc);
  ...
  private enemyLocations(
    ...
    for (const action of this.db.Actions) {
      if (
        ...
        action["Target ID"] !== enemy["Enemy ID"]
      )
        continue;
      for (const loc of actionLocations.get(action["Action ID"]) ?? [])
        add(loc);
    }
...
function isOreGemTable(action: ActionRow, tableId: string): boolean {
  return (
    action["Relevant Skill ID"] === MINING_SKILL_ID &&
    ORE_GEM_TABLE_IDS.has(tableId)
  );
}
...
function withEffectiveDropRates(
  drops: CodexItemRef[],
  tableChance: number | null,
): CodexItemRef[] {
  ...
  return drops.map((drop) => ({
    ...drop,
    dropRatePercent:
      drop.weight != null && totalWeight > 0
